...
export class SupplierTypeListComponent implements OnInit {
  ...
  constructor(private _sidebarService: SidebarService, private _router: Router, private _supplierTypeService: SupplierTypeService, private _localStorage: LocalStorageService) {
  ...
  goToUpdateSupplierType(data: any){
    this._router.navigate(['/supplier-types','update',data.supplierTypeId]);
  }

  deleteSupplierType(data: any){
    swal({
      title: "Atención",
    ...
    .then((willDelete) => {
      if (willDelete) {
        this._supplierTypeService.deleteSupplierType(data.supplierTypeId).subscribe((resp)=>{
          if(resp.json()){
