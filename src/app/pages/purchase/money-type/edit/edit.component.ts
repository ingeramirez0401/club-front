...
export class MoneyTypeEditComponent implements OnInit {
  forma:FormGroup;
  userProfile: UserProfileModel;
  moneyId: number = 0;
  constructor(private _sidebarService: SidebarService, private _router: Router, private _localStorage: LocalStorageService, private _appService: AppService, private _moneyService: MoneyTypeService, private _activatedRoute: ActivatedRoute) {
    this.userProfile = this._localStorage.getParsedObject("auth-data");
    this.formInit();

    this._activatedRoute.params.subscribe((params)=>{
      this.moneyId = params['id'];
      this._moneyService.getMoneyType(this.moneyId).subscribe((result)=>{
  ...
  formInit() {
    this.forma = new FormGroup({
      'MoneyTypeId': new FormControl(0, Validators.required),
  ...
  saveChanges() {
    ...
    this._moneyService.updateMoneyType(this.forma.value)
    .subscribe(result => {
      $('body').loading('stop');
  ...
  goToList() {
    this._router.navigate(['/money-types','list']);
  }
}
