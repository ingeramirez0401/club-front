...
export class DocumentTypesEditComponent implements OnInit {
  ...
  documentId: number = 0;
  constructor(private _sidebarService: SidebarService, private _router: Router, private _localStorage: LocalStorageService, private _appService: AppService, private _documentService: DocumentTypesService, private _activatedRoute: ActivatedRoute) {
    this.userProfile = this._localStorage.getParsedObject("auth-data");
    this.formInit();

    this._activatedRoute.params.subscribe((params)=>{
      this.documentId = params['id'];
      this._documentService.getDocument(this.documentId).subscribe((result)=>{
  ...
  saveChanges() {
    ...
    this._documentService.updateDocument(this.forma.value)
    .subscribe(result => {
      $('body').loading('stop');
