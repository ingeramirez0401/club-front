...
export class ProductTypeEditComponent implements OnInit {
  ...
  constructor(private _sidebarService: SidebarService, private _router: Router, private _localStorage: LocalStorageService, private _appService: AppService, private _productTypeService: ProductTypeService, private _activatedRoute: ActivatedRoute) {
    this.userProfile = this._localStorage.getParsedObject("auth-data");
    this.formInit();

    this._activatedRoute.params.subscribe((params)=>{
      this.productTypeId = params['id'];
      this._productTypeService.getProductType(this.productTypeId).subscribe((result)=>{
  ...
  saveChanges() {
    ...
    this._productTypeService.updateProductType(this.forma.value)
    .subscribe(result => {
      $('body').loading('stop');
