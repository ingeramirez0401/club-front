...
import { SidebarService, LocalStorageService } from '../../../../services/services.index';
...
export class SystemsListComponent implements OnInit {
  ...
  constructor(private _sidebarService: SidebarService, private _router: Router, private _systemService: SystemService, private _localStorage: LocalStorageService) {
  ...
  loadAllSystems(){
    ...
    this.systemList = [];
    this._systemService.getAllSystems().subscribe((result)=>{
      $('body').loading('stop');
      this.systemList = result.json().data;
      if(this.systemList){
